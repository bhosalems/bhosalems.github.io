...
At the time of sampling, the diffusion score $$ \epsilon_\theta(z_t, t, c) $$ is modified to include the adversarial gradient of the classifier {% cite NEURIPS2021_49ad23d1 --file 2024-11-28-diff %}:
...
In classifier-free guidance {% cite classifier_free --file 2024-11-28-diff %}, a single neural network is used to parameterize both the unconditional denoising diffusion model $$ p_\theta(z) $$ and the conditional denoising diffusion model $$ p_\theta(z | c) $$. 
While training, the unconditional model receives a null token, $$ \Phi $$, as $$ c $$ randomly with some probability $$ p_\text{uncond} $$, set as a hyperparameter. 
...
###### <span style="font-weight: bold;">Step 2: Predicted Noise $$\epsilon_{theta}(x_t, t)$$</span>
...
```
That briefly settles diffusion models, for more details please visit {% cite chan2024tutorial --file 2024-11-28-diff %}.
